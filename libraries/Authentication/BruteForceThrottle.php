...
class BruteForceThrottle
{
     ...
     * @param callable(int $failedCount):void|null $onMissOneChance
    ...
    public function __construct(
        ...
        if (null !== $sessionLimit and $sessionLimit > $totalLimit) {
            throw new \InvalidArgumentException("the session based limit should not bigger than total limit, total limit: {$totalLimit} session limit: {$sessionLimit}");
        }
        ...
        } elseif (!is_string($this->options['cache-name'])) {
            throw new \InvalidArgumentException('the cache-name option should be a non-empty string!, given: '.Json\encode($this->options['cache-name']));
        }

        if (isset($this->options['ignore-ips']) and !is_array($this->options['ignore-ips'])) {
            throw new \InvalidArgumentException('the ignore-ips option should be a array of string!, given: '.Json\encode($this->options['ignore-ips']));
        }
    ...
    public function loseOneChance(): void
    {
        ...
        if ($this->onMissOneChance) {
            call_user_func(
                ...
                null !== $sessionBasedTriesCount ? $sessionBasedTriesCount + 1 : $totalTriesCount + 1
            );
        }
    }
    ...
    {
        if (
            isset($this->options['ignore-ips'])
            and $this->options['ignore-ips']
            and in_array(self::getClientIP(), $this->options['ignore-ips'])
        ) {
            return true;
        }
